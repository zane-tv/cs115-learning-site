...
function ensureKatex(){
  ...
  katexPromise=loadScript(core,()=>!!window.katex&&typeof window.katex.render==='function')
    ...
    .catch(error=>{
      katexPromise=null;
      console.warn('Không tải được KaTeX',error);
      throw error;
    });
  return katexPromise;
}
...
if(!sid){
  sid=(crypto.randomUUID?.()||`${Date.now()}${Math.random()}`).replaceAll('-','');
  set(KEYS.sid,sid);
}
let chat=[];
try{
  chat=JSON.parse(get(KEYS.chat)||'[]')
...
const box=$('.ai-box');
const openBtn=$('.ai-open');
const keyInput=$('.ai-key-input');
const modelSelect=$('.ai-select');
const msgs=$('.ai-msgs');
const text=$('textarea');
...
function setModels(list,preferred){
  ...
  for(const item of list){
    ...
    o.value=id;
    o.textContent=typeof item==='string'?item:(item.label||id);
...
function note(message,isBad=false){
...
function normalizeMathEscapes(value){
  ...
  const cleanBody=b=>b
    ...
    .replace(/\s*\n\s*/g,' ')
    .trim();
  s=s.replace(/\\\[([\s\S]*?)\\\]/g,(_,b)=>'\\['+cleanBody(b)+'\\]');
  s=s.replace(/\$\$([\s\S]*?)\$\$/g,(_,b)=>'\\['+cleanBody(b)+'\\]');
  s=s.replace(/\\\(([\s\S]*?)\\\)/g,(_,b)=>'\\('+cleanBody(b)+'\\)');
  s=s.replace(/\$([^$\n]+?)\$/g,(_,b)=>'$'+cleanBody(b)+'$');
  return s;
...
function appendInline(parent,source){
  ...
  while((m=token.exec(source))){
    ...
    if(mathToken){
      ...
      e.textContent=t.slice(1,-1);
      parent.append(e);
    }else if(t.startsWith('**')){
      const e=document.createElement('strong');
      appendInline(e,t.slice(2,-2));
      parent.append(e);
    }else if(t.startsWith('~~')){
...
function renderMarkdown(markdown){
  ...
  while(i<lines.length){
    ...
    if(fence){
      ...
      if(language)c.dataset.language=language;
    ...
    if(/^\s*>\s?/.test(line)){
      ...
      const content=[];
      while(i<lines.length&&/^\s*>\s?/.test(lines[i]))content.push(lines[i++].replace(/^\s*>\s?/,''));
      q.append(makeParagraph(content));
      fragment.append(q);
      continue;
    }
...
function renderMath(container){
  ensureKatex()
    ...
    .catch(err=>console.warn('KaTeX unavailable',err));
}

function bubble(role,content,isBad=false){
  const d=document.createElement('div');
  d.className='ai-msg '+(isBad?'bad':role);
  if(role==='assistant'&&!isBad){
    const normalized=normalizeMathEscapes(content);
    d.classList.add('ai-render');
    d.append(renderMarkdown(normalized));
    renderMath(d);
  }else{
...
function renderChat(){
  ...
  if(!chat.length){
    bubble('assistant','Chào bạn! Hãy nhập OpenAI API key trong **Cấu hình OpenAI**. Tôi có thể trình bày Markdown và công thức như $A^T A$, hoặc khối công thức:\n\n$$\\nabla f(x)=0$$');
  }else{
    chat.forEach(x=>bubble(x.role,x.content));
  }
}
renderChat();
...
function show(value){
  ...
  if(value){
    if(!keyInput.value)$('.ai-settings').open=true;
    setTimeout(()=>{(keyInput.value?text:keyInput).focus()},0);
  }
}
openBtn.onclick=()=>show(box.hidden);
$('.ai-close').onclick=()=>show(false);
...
keyInput.addEventListener('input',()=>{
  ...
  if(v)set(KEYS.api,v);else del(KEYS.api);
});
$('.ai-forget').onclick=()=>{
  keyInput.value='';
  del(KEYS.api);
  note('Đã xóa API key khỏi localStorage.');
  keyInput.focus();
};
$('.ai-reset').onclick=()=>{
  chat=[];
  del(KEYS.chat);
  renderChat();
  note('Đã xóa cuộc trò chuyện.');
};

$('.ai-models').onclick=async function(){
  const key=keyInput.value.trim();
  if(!key){note('Hãy nhập API key.',true);return keyInput.focus()}
  this.disabled=true;
  note('Đang kiểm tra key và tải danh sách model…');
  try{
    ...
    note(`Key hợp lệ · tìm thấy ${data.models?.length||0} model văn bản.`);
  }catch(err){
    note(err.message||'Không tải được model.',true);
  }finally{
    this.disabled=false;
  }
};
...
root.querySelector('form').onsubmit=async e=>{
  ...
  if(!key){
    ...
    note('Hãy nhập API key trước.',true);
    return keyInput.focus();
  }

  set(KEYS.api,key);
  chat.push({role:'user',content:question.slice(0,6000)});
  chat=chat.slice(-20);
  set(KEYS.chat,JSON.stringify(chat));
  ...
  try{
    ...
    note(`Đã trả lời bằng ${data.model||modelSelect.value}.`);
  }catch(err){
    wait.remove();
    bubble('assistant',err.message||'Không thể nhận câu trả lời.',true);
    note(err.message||'Có lỗi xảy ra.',true);
  }finally{
    send.disabled=false;
    text.focus();
  }
